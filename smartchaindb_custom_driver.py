...
class SmartChainDBDriver:
    ...
    def prepare_request_return_transaction(self, signers, asset_id, sell_transaction_id, metadata=None):
        ...
        # Create proper input that references the SELL transaction output (buyer now owns the asset)
        fulfills_link = TransactionLink.from_dict({
            'output_index': 0,  # Asset output (first output is the asset to buyer)
            'transaction_id': sell_transaction_id,
        })
        
        request_return_input = Input.generate([signers])
        request_return_input.fulfills = fulfills_link
